...
- All other tools (`subagent`, `todo`, `memory`, `memory_search`, `session_search`, `video_extract`, `skill_manage`, `plan`, etc.) use the generic fallback in `patchTool()`
...
- Hide-toggle parity (Ctrl+T): the patched `AssistantMessageComponent.updateContent` MUST check `this.hideThinkingBlock` before rendering a `ThinkingBlock`. When hidden, render the duration-aware label from `docs/plans/thinking.md`. While the message is still streaming, the verb is `Thinking for Ns` (live counter, uses `Date.now() - startMs` while `endMs` is unset). Once `message_end` fires and `endMs` is set, the verb flips to `Thought for Ns` and the duration locks. Sub-second durations round up to `1s` (per user preference) rather than the upstream framework's `Thought for <1s`. For pre-existing entries that we never observed (no Symbol-keyed property on the message), fall back to `✻ Thought...`. The label is computed via `getNativeThinkingLabel(message, fallback)` which reads a non-enumerable Symbol-keyed property (`Symbol.for("compactui.nativeThinkingTiming")`) directly on the AssistantMessage. The property travels with the message reference across streaming updates, `message_end`, `_replaceMessageInPlace`, and `buildSessionContext()` rebuilds — a WeakMap is unnecessary and reference-identity gotchas can't drop the entry. `recordNativeThinkingStart(message)` is called on the first sighting inside the patched `updateContent` and MUST run outside the hide branch so the property exists during streaming (not just at rebuild time). `recordNativeThinkingEnd(message)` is called from a `message_end` event hook. Color matches the `✻ Worked for Xs` footer (`assistant-footer.ts`): raw DIM_GREY (`\x1b[38;2;140;140;140m ... \x1b[0m`), no italic. Skipping this branch breaks the Ctrl+T toggle — the notification flips but the thinking content stays visible
...
- Verify Ctrl+T toggle: with a thinking-heavy response already in chat, press Ctrl+T — the status reads `Thinking blocks: hidden`, and every previously visible thinking block collapses to a single dim-grey line in the same DIM_GREY as `✻ Worked for Xs`. Current entries must show `✻ Thinking for Ns` while the agent is still streaming (live counter), then flip to `✻ Thought for Ns` and lock once `message_end` fires. Sub-second blocks read `✻ Thinking for 1s` / `✻ Thought for 1s`. Pre-existing entries (no WeakMap data) must read `✻ Thought...`. Pressing Ctrl+T again expands them back to full `ThinkingBlock` rendering. If a model-emitted `✻ Thought for Xs` footer inside the thinking content itself stays visible after toggling hidden, the patched `updateContent` is not respecting `this.hideThinkingBlock`
