...
export function loadConfig(configPath = DEFAULT_CONFIG_PATH): MemoryConfig {
  try {
    if (fs.existsSync(configPath)) {
      ...
      // Validate char limits are positive
      if (typeof config.memoryCharLimit !== "number" || config.memoryCharLimit <= 0) {
        config.memoryCharLimit = DEFAULT_MEMORY_CHAR_LIMIT;
      }
      if (typeof config.userCharLimit !== "number" || config.userCharLimit <= 0) {
        config.userCharLimit = DEFAULT_USER_CHAR_LIMIT;
      }
      if (typeof config.projectCharLimit !== "number" || config.projectCharLimit <= 0) {
        config.projectCharLimit = DEFAULT_PROJECT_CHAR_LIMIT;
      }
      if (typeof config.nudgeInterval !== "number" || config.nudgeInterval < 1) {
        config.nudgeInterval = DEFAULT_NUDGE_INTERVAL;
      }
      if (typeof config.flushMinTurns !== "number" || config.flushMinTurns < 1) {
        config.flushMinTurns = DEFAULT_FLUSH_MIN_TURNS;
      }
      return config;
    }
  } catch (err: any) {
    // Log non-parse errors (EACCES, EISDIR, etc.) but still fall back to defaults
    if (err?.code && err.code !== "ENOENT" && err.code !== "SyntaxError") {
      console.warn(`[hermes-memory] Config file error (${err.code}): ${configPath}`);
    }
  }
