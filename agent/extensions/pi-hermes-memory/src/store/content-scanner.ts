...
const MEMORY_THREAT_PATTERNS: Array<{ pattern: RegExp; id: string }> = [
  ...
  { pattern: /base64\s+(--decode|-d)\s/i, id: "exfil_base64_decode" },
  { pattern: /xxd\s+-[ps]/i, id: "exfil_xxd" },
  { pattern: /echo\s+\S+\s*\|\s*(curl|wget|nc|ncat|socat)/i, id: "exfil_pipe_network" },
];
...
const SECRET_PATTERNS: Array<{ pattern: RegExp; id: string; severity: "high" | "medium" }> = [
  ...
  { pattern: /\bANTHROPIC_API_KEY\s*[=:]\s*\S+/i, id: "env_anthropic_key", severity: "medium" },
  { pattern: /\bOPENAI_API_KEY\s*[=:]\s*\S+/i, id: "env_openai_key", severity: "medium" },
  { pattern: /\bOPENROUTER_API_KEY\s*[=:]\s*\S+/i, id: "env_openrouter_key", severity: "medium" },
  { pattern: /\bGITHUB_TOKEN\s*[=:]\s*\S+/i, id: "env_github_token", severity: "medium" },
  { pattern: /\bAWS_SECRET_ACCESS_KEY\s*[=:]\s*\S+/i, id: "env_aws_secret", severity: "medium" },
  { pattern: /\bDATABASE_URL\s*[=:]\s*\S+/i, id: "env_database_url", severity: "medium" },
...
const INVISIBLE_CHARS = new Set([
  ...
  '\u3164', // HANGUL FILLER
  '\u115f', '\u1160', // CHOSUNG FILLER, JUNGSEONG FILLER
  '\u00ad', // SOFT HYPHEN
  '\u200e', '\u200f', // LTR/RTL MARK
