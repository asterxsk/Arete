...
export class MemoryStore {
  ...
  async loadFromDisk(): Promise<void> {
    ...
    // Deduplicate by content (stripping metadata), keeping most recent
    this.memoryEntries = this.deduplicateEntries(this.memoryEntries);
    this.userEntries = this.deduplicateEntries(this.userEntries);
    this.failureEntries = this.deduplicateEntries(this.failureEntries);
  ...
  /**
   * Deduplicate entries by stripped content. When duplicates exist,
   * keep the one with the most recent last_referenced date.
   */
  private deduplicateEntries(entries: string[]): string[] {
    const seen = new Map<string, { raw: string; lastReferenced: string }>();
    for (const entry of entries) {
      const decoded = this.decodeEntry(entry);
      const existing = seen.get(decoded.text);
      if (!existing || decoded.lastReferenced > existing.lastReferenced) {
        seen.set(decoded.text, { raw: entry, lastReferenced: decoded.lastReferenced });
      }
    }
    return Array.from(seen.values()).map((v) => v.raw);
  }
