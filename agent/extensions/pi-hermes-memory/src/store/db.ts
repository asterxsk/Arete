...
function loadDatabaseCtor(): DatabaseCtor {
  // Resolve from extension root (package.json location) to find node_modules
...
export class DatabaseManager {
  ...
  private migrateLegacyMemoriesTargetConstraint(db: DatabaseLike): void {
    // Create backup before migration — best-effort, non-blocking
    try {
      const backupName = `memories_backup_${Date.now()}`;
      db.exec(`CREATE TABLE IF NOT EXISTS "${backupName}" AS SELECT * FROM memories`);
    } catch {
      // Backup is best-effort — don't block migration
    }
