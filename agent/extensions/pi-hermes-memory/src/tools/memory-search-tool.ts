...
Returns matching memory entries with project context and dates.`,
    ...
    execute: withDurationCapture(async (_id: string, args: { query: string; project?: string; target?: string; category?: string; limit?: number }) => {
      ...
      for (const entry of results) {
        ...
        const targetLabel = entry.target === 'user' ? 'User' : entry.target === 'failure' ? 'Failure' : 'Memory';
