...
export function scheduleLiveSessionIndex(
  ...
  if (state.inProgress) {
    // Queue a re-index after current one completes
    pendingReindex = true;
    return true;
  }
  ...
  state.promise = new Promise<void>((resolve) => {
    setTimeoutFn(() => {
      try {
        ...
        state.promise = null;
        // If events were queued during indexing, trigger another pass
        if (pendingReindex) {
          pendingReindex = false;
          scheduleLiveSessionIndex(dbManager, sessionManager, options);
        }
        resolve();
      }
    }, delayMs);
