...
export default function activate(pi: ExtensionAPI) {
	...
	(globalThis as any).__pi_extension_features?.push({
		...
		description: "Search Google Images via Custom Search API with inline thumbnails for visual selection",
		tools: ["google_image_search"],
	});
	...
	pi.registerTool({
		...
		renderResult(result, { isPartial, expanded }) {
			...
			if (result.isError || text.startsWith("Error")) return new CompactResult({ toolName: "google_image_search", argsLine: text ? text.split("\n")[0] : "error", state: "error" });
			...
			return new CompactResult({
				toolName: "google_image_search",
				argsLine: query,
				state: "done",
				previewLines,
				footer: `${results.length} images`,
				expanded,
			});
		},
	});
