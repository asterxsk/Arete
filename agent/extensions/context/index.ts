...
function computeBreakdown(ctx: any): ContextBreakdown | null {
	...
	let sysPromptBaseTokens = 0;
	let sysPromptToolsTokens = 0;
	let sysPromptSkillsTokens = 0;
	let sysPromptGuidelinesTokens = 0;
	let sysPromptDocsTokens = 0;
	...
	try {
		...
		if (sysPrompt) {
			const extractRegex = (regex: RegExp) => {
				const match = sysPrompt.match(regex);
				return match ? estimateStringTokens(match[0]) : 0;
			};
			
			sysPromptToolsTokens = extractRegex(/Available tools:\n[\s\S]*?(?=\n\nIn addition to|\n\nGuidelines:|$)/) || extractRegex(/<tools>[\s\S]*?<\/tools>/);
			sysPromptGuidelinesTokens = extractRegex(/Guidelines:\n[\s\S]*?(?=\n\nPi documentation|\n\n<project_context>|\n\nThe following skills|$)/) || extractRegex(/<guidelines>[\s\S]*?<\/guidelines>/);
			sysPromptDocsTokens = extractRegex(/Pi documentation[\s\S]*?:\n[\s\S]*?(?=\n\n<project_context>|\n\nThe following skills|\nCurrent date:|$)/);
			sysPromptSkillsTokens = extractRegex(/The following skills provide specialized instructions[\s\S]*?<\/available_skills>/) || extractRegex(/<skills>[\s\S]*?<\/skills>/) || extractRegex(/<available_skills>[\s\S]*?<\/available_skills>/);
			
			const totalSys = estimateStringTokens(sysPrompt);
			sysPromptBaseTokens = Math.max(0, totalSys - sysPromptToolsTokens - sysPromptSkillsTokens - sysPromptGuidelinesTokens - sysPromptDocsTokens);
		}
	} catch {
	...
	addCat("sys_base", "Sys Prompt (Base)", sysPromptBaseTokens,
		(th, t) => ansi256Fg(141, t), ansi256Bg(141, "  ")); // Purple

	addCat("sys_tools", "Sys Prompt (Tools)", sysPromptToolsTokens,
		(th, t) => ansi256Fg(140, t), ansi256Bg(140, "  ")); // Darker Purple

	addCat("sys_skills", "Sys Prompt (Skills)", sysPromptSkillsTokens,
		(th, t) => ansi256Fg(139, t), ansi256Bg(139, "  ")); // Even Darker Purple

	addCat("sys_guides", "Sys Prompt (Guides)", sysPromptGuidelinesTokens,
		(th, t) => ansi256Fg(138, t), ansi256Bg(138, "  ")); // Darkest Purple
		
	addCat("sys_docs", "Sys Prompt (Docs)", sysPromptDocsTokens,
		(th, t) => ansi256Fg(137, t), ansi256Bg(137, "  ")); // Darkest Purple
	...
	// Use the real context usage if available AND greater than 0, otherwise use our estimate
	const totalTokens = (usage.tokens ?? 0) > 0 ? usage.tokens! : usedFromCategories;
...
function buildOverlay(
	...
	// Copilot quota line (read from the globalThis bridge installed by
	// this extension on session_start). Falls back to a dim placeholder
	// when the bridge is not available or the fetch is still in flight.
	const copilotStatus = readCopilotUsage?.();
	const copilotLine =
		copilotStatus === undefined
			? "copilot: bridge not installed"
			: copilotStatus === "loading"
				? "copilot: loading..."
				: copilotStatus === "not-logged-in"
					? "copilot: not logged in"
					: buildCopilotUsageLine(copilotStatus);
	lines.push(row(theme.fg("accent", theme.bold("Session Stats"))));
	lines.push(row(theme.fg("muted", copilotLine)));
...
import { installCopilotUsageBridge, destroyCopilotUsageBridge, readCopilotUsage, buildCopilotUsageLine } from "./copilot-usage.ts";

export default function (pi: ExtensionAPI) {
	// Self-register in global feature registry
	(globalThis as any).__pi_extension_features?.push({
		name: "context",
		description: "Visualize context window usage as a colored grid overlay with per-category breakdowns",
		commands: ["/context"],
	});
