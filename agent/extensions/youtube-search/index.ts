...
export default function (pi: ExtensionAPI) {
	...
	(globalThis as any).__pi_extension_features?.push({
		name: "youtube-search",
		description: "Search YouTube via yt-dlp with structured metadata (title, duration, views, channel, thumbnail)",
	...
	pi.registerTool({
		...
		renderResult(result, { isPartial, expanded }) {
			...
			if (isPartial) return new CompactResult({ toolName: "youtube_search", argsLine: "searching...", state: "pending" });
			const content = result.content[0];
			const text = content?.type === "text" ? content.text : "";
			if (result.isError || text.startsWith("Error")) return new CompactResult({ toolName: "youtube_search", argsLine: text ? text.split("\n")[0] : "error", state: "error" });
			
			const details = result.details as any;
			const query = details?.query || "done";
			const results = details?.results || [];
			
			let previewLines: string[] | undefined;
			if (expanded) {
				previewLines = results.map((r: any) => {
					const line = `${r.title} - ${r.url} (${r.duration_str})`;
